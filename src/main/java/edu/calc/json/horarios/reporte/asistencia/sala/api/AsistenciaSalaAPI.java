package edu.calc.json.horarios.reporte.asistencia.sala.api;
...
import edu.calc.json.horarios.reporte.asistencia.sala.model.WrapperAsistenciaAlumno;
import edu.calc.json.horarios.reporte.asistencia.sala.service.AsistenciaSalaService;
import edu.calc.json.horarios.utils.UtilDate;
...
public class AsistenciaSalaAPI {
    ...
    public WrapperAsistenciaAlumno getAlumnosByScheduleAndUser(
            ...
            @ApiParam(value = "Fecha inicio", defaultValue = Constant.TODAY) @RequestParam(value = "fecha-inicio", defaultValue = Constant.TODAY, required = false) String fechaInicio,
            @ApiParam(value = "Fecha Fin", defaultValue = Constant.TODAY) @RequestParam(value = "fecha-fin", defaultValue = Constant.TODAY, required = false) String fechaFin) throws Exception {
        WrapperAsistenciaAlumno asistenciaAlumno = new WrapperAsistenciaAlumno();



        if (fechaInicio.equalsIgnoreCase(Constant.TODAY)) {
            Date today = new Date();
            fechaInicio = UtilDate.convertDateToString(today, UtilDate.PATTERN_GUION);
        }

        if (fechaFin.equalsIgnoreCase(Constant.TODAY)) {
            Date today = new Date();
